...
<font color=red> 这个section，以及后面的Section 13.3暂时就是没有看的了，一个是觉得暂时是不需要用到这部分的内容。第二个，这个section提及到就是需要创建ONNX的节点，这里我就是有点晕了，不知所云（和代码部分没有对的上号，也是因为时间的关系，就暂时没有继续是深究下去了）。这两个section的内容，就是留着以后，再过一遍，对于现阶段，这个部分的内容显示的并没有那么的重要。毕竟不可能没有学会走路，都是想跑了。</font>
