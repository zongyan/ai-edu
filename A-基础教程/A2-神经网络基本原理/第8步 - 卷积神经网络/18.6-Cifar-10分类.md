...
Cifar 是加拿大政府牵头投资的一个先进科学项目研究所。Hinton、Bengio和他的学生在2004年拿到了 Cifar 投资的少量资金，建立了神经计算和自适应感知项目。这个项目结集了不少计算机科学家、生物学家、电气工程师、神经科学家、物理学家、心理学家，加速推动了 Deep Learning 的进程。从这个阵容来看，DL 已经和 ML 系的数据挖掘分的很远了。Deep Learning 强调的是自适应感知和人工智能，是计算机与神经科学交叉；Data Mining 强调的是高速、大数据、统计数学分析，是计算机和数学的交叉。<font color=green> 这里给出了deep learning和data mining之间的一些区别。</font>
...
<font color=blue> 到目前为止，就是已经是把mnist，fashion-mnist，Cifar-10，Cifar-100之间的区别搞清楚了，mnist&fashion-mnist都是单通道的灰色图，但是后两者都是三通道的彩色图。另外前两者只是有10个分类，Cifar也是10个分类，但是最后一个数据集是100个分类。</font>
...
1. 先用卷积->激活->卷积->激活->池化->丢弃层，做为第一梯队，卷积核32个；<font color=blue> 注意一下dropout layer的位置。</font>
2. 然后再用卷积->激活->卷积->激活->池化->丢弃层做为第二梯队，卷积核64个；
3. Flatten和Dense相当于把池化的结果转成Nx512的全连接层，N是池化输出的尺寸，被Flatten扁平化了；<font color=red> ToDo:这个在PyTorch中也是需要的吗？</font>
4. 再接丢弃层，避免过拟合；
...
<font color=green> 这个部分的内容就仅仅是过了文字部分，代码因为我暂时没有这个tensorflow，然后也是（学校配的笔记本）没有GPU，所以就是没有看代码的了。但是相同文件路径下面的Pytorch.py这个和我以后的代码是非常的接近的，这个代码是非常的有用的。

换句话说，所有的章节里面，都是会用相应的pytorch版本的代码，这个是一件非常好的事情，可以帮助我快速的上手。</font>
