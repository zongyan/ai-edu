...
表18-3中，左侧为卷积核的作用，右侧为某个特征对于5种形状的判别力度，0表示该特征无法找到，1表示可以找到该特征。<font color=green> 需要注意的是，这个他变了18.3，是针对的一个层的卷积层的8个卷积核。</font>
...
当然，神经网络可能不是按照我们分析的顺序来判定形状的，这只是其中的一种解释路径，还可以有很多其它种路径的组合，但最终总能够把5种形状分开来。<font color=green>（其实这句话的解释是非常的好的，这个仅仅是作为神经元的一种解释，言之有理就是可以的了）。</font>

<font color=red> todo: 但是我这里就是有了一个疑惑，对于每一层的卷积核的个数选择， 以及卷积核的维度选择，这个应该是怎么决定的呢？</font>
...
1. 我们使用了3x3的卷积核，如果用5x5的卷积核，但是在其它参数不变的情况下，其效果会不会更好？<font color=green> 从evolution of accuracy上面来看，两者并没有特别大的区别。另外，console的数值也是从0.962变成了0.959，并没有太大的变化。</font><font color=red> 也就是说，改变卷积核的尺寸，对性能并不会有特别大的影响。简单的搜索了一下，应该还是和网络的能力有关系。卷积核是小而深比较好，大而浅就是不太好，就是说卷积核尽量小点，然后个数多一些的了。</font>
2. 可以建立一个数据集，只包括正圆、椭圆、正方形、矩形等四种形状，看看卷积神经网络是不是能分辨出来。<font color=green> 这一些的数据，都是在本地使用python代码生成的，只要把不想要的形状的相应的代码，自动删除就是可以的了。</font>

<font color=green> 这个代码的思路，其实和Section 18.1的思路是非常的类似的，只不过Section 18.1是用来检测color，这个section是用来检测这个形状的了。</font>
