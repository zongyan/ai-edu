...
```Python
    ...
    # 用输入数据乘以回传入的误差矩阵,得到卷积核的梯度矩阵 ---- 计算卷积核的梯度矩阵
    def _calculate_weightsbias_grad(self, dz):
        self.WB.ClearGrads()
        # 先把输入矩阵扩大，周边加0
        (pad_h, pad_w) = calculate_padding_size(...) # 为什么计算梯度矩阵，需要padding?为了保证得到的梯度矩阵和weights的维度一致
        input_padded = np.pad(self.x, ...)
        # 输入矩阵与误差矩阵卷积得到权重梯度矩阵
        (self.WB.dW, self.WB.dB) = calcalate_weights_grad(...)
        self.WB.MeanGrads(self.batch_size)

    # 用输入误差矩阵乘以（旋转180度后的）卷积核 ---- 计算误差矩阵
...
<font color=green> 不管是在计算梯度矩阵，还是计算误差矩阵的时候，其实最好都是需要检查一下这个输出矩阵，是不是符合对应的要求，比如说梯度矩阵的结果是不是符合权重矩阵的维度；误差矩阵是不是符合本层（正向传播时）的输入矩阵的维度。 </font>
...
dB=
 [[6.]]
```
计算dW（这一步和全连接层完全相同）<font color=green> 参见Section 17.3的(11) -- 因为是按照全连接层的计算方式，所以dW是二维的，然后每一行代表神经元的个数，每一列代表特征值的个数。col_x的每一行代表一个“样本”，每一列代表一个“特征值”，所以需要转置。同时，delta_in_2d就是按照这一种“类似于列向量”的方式在每一层之间进行传递的</font>：
```Python
dW = np.dot(self.col_x.T, delta_in_2d) / self.batch_size
```
得到：
...
这个结果和最后一步delta_out的结果完全一致。<font color=green> 这里需要结合图片，从col2img的过程中，有一个叠加过程的存在。</font>
...
delta_in是本层的误差输入矩阵，它的形状应该和本层的前向计算结果一样<font color=blue>（这句话还是非常的重要的了，算是贯穿了整个的误差矩阵&梯度矩阵计算过程）</font>。在本例中，误差输入矩阵的形状应该是：(batch_size * output_channel * output_height * output_width) = (2 x 2 x 2 x 2)：
```
delta_in=
(样本1)
    (通道1)
...
```
<font color=green>delta_out中的样本1的通道1的第1行的第2列数据112，其实是dcol中的第1行的第2个数&第2行的第1个数的求和结果，之所以要相加，在从col2img的过程中，就是会出现这一种某一列或者某一行重合的现象，此时就是需要进行相加，如图17.30所示</font>
...
那么我们能不能混用numba方法的前向计算和img2col方法的反向传播呢？不行。因为img2col方法的反向传播需要用到其正向计算方法中的两个缓存数组，一个是输入数据的矩阵变换结果 <font color=green> (i.e. col_x -- 计算梯度矩阵)</font>，另一个是权重矩阵变换结果<font color=green> (i.e. col_w -- 计算误差矩阵)</font>，所以img2col方法必须正向反向配合使用。
...
<font color=red> ToDo: 这个高维度的array真的是一件非常头疼的事情，相比于二维，等会儿还是需要再过一遍代码，然后加深一下理解的了，要不然还是有一种一知半解的感觉。

另外，这里也是提到了，在计算梯度矩阵和误差矩阵的时候，其思路就是和全连接层是一样的，我觉得有必要再review一遍这个全连接层的计算方式了。

另外，这个部分的代码，如果有必要的话，下周一的时候需要再过一遍的了。------>（再次过了一遍之后，就是对这个img2col和col2img就是有了更加深刻的理解的，也是在这个过程中，把全连接的反向传播，numpy.reshap，weight矩阵在全连接层是二维，在卷积层是四维）。

最后，作者说的一句话是非常重要的，之所以使用卷积优化的方式，主要目的就是在于加速，当然，使用numba可以更进一步的加速。同时的呢，img2col和col2img的精髓就是在于，将卷积层的前向计算（比如权重矩阵，每一层的输入矩阵）&反向计算（比如误差矩阵，梯度矩阵）都是转变成vector的形式，然后就是可以使用全连接层的思维，进行计算了（其实计算公式就是和全连接层的计算公式是一样的），然后接着就是需要做相应的维度变换操作，就是可以的了。
</font>
