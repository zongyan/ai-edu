...
卷积神经网络是现在深度学习领域中最有用的网络类型，尤其在计算机视觉领域更是一枝独秀。卷积神经网络从90年代的LeNet开始，沉寂了10年，也孵化了10年，直到2012年AlexNet开始再次崛起，后续的ZF Net、VGG、GoogLeNet、ResNet、DenseNet，网络越来越深，架构越来越复杂，解决反向传播时梯度消失的方法也越来越巧妙。下面让我们一起学习一下这些经典的网络模型。$^{[1,2]}$ <font color=red> ToDo: 无聊的时候，可以跑跑这一些的经典的卷积神经网络。</font>
...
1. 输入为单通道32x32灰度图
2. 使用6组5x5的过滤器，每个过滤器里有一个卷积核，stride=1，得到6张28x28的特征图 <font color=green>（卷积层维度： 6 x 1 x 5 x 5）</font>
3. 使用2x2的池化，stride=2，得到6张14x14的特征图
4. 使用16组5x5的过滤器，每个过滤器里有6个卷积核，对应上一层的6个特征图，得到16张10x10的特征图 <font color=green>（卷积层维度： 16 x 6 x 5 x 5）</font>
5. 池化，得到16张5x5的特征图
6. 接全连接层，120个神经元
7. 接全连接层，84个神经元
8. 接全连接层，10个神经元，softmax输出

如今各大深度学习框架中所使用的LeNet都是简化改进过的LeNet-5（5表示具有5个层），和原始的LeNet有些许不同，比如把激活函数改为了现在很常用的ReLu。LeNet-5跟现有的conv->pool->ReLU的套路不同，它使用的方式是conv1->pool->conv2->pool2再接全连接层，但是不变的是，卷积层后紧接池化层的模式依旧不变。<font color=red> 上面介绍LeNet的文字部分，就是没有介绍激活层，也许他只是在全连接层使用了激活函数，这个就是需要看原文了。不过这里可以确定的是，现有框架下的LeNet的方式（conv->pool->ReLU）</font>

<font color=blue> 其实，上面的第二个步骤，也是写明了，就是每一个过滤器就是有一个卷积核的，从某种程度上，作者自己本身也是考虑了这个过滤器和卷积核之间的关系的。所以，第一遍阅读的时候，关于卷积核&过滤器的维度问题，可能就是有部分的信息是忽略了，第二遍阅读的时候，注意一下。</font>

<font color=green> 关于LeNet的介绍，我还是看的非常的明白的了，特别是关于卷积核，过滤器的维度部分，还是非常清晰的。</font>
...
图18-2 AlexNet模型结构图 <font color=green>（因为是两块GPU并行计算的，所以应该就是有上下两个部分，然后就是在最后合并在一起，这张图就是有点欠缺，只是完整的画出了下半部分，上半部分有部分欠缺了。）</font>
...
<font color=green> 关于AlexNet的介绍，有点和本书的的思路出现了一些的偏差，比如说在LeNet中，就是提到了filter & kernal两个概念，然后我就是可以很好的看明白这个输出数据的维度。但是，在介绍这个AlexNet的时候，比如第二个bull point，作者就是仅仅是提到了有96核，然后没有提出有多少的filer，但是根据输出维度96 x 55 x 55的理解，就是可以知道，其实这里的“核”的概念，就是本书的filter的概念了。</font> <font color=red> 这一点就是非常的奇怪，为什么不说清楚filter & kernal的所有具体数值呢。</font>
...
<font color=green> 看完每一层的演变过程，顿时就是觉得则个好神奇呀，没有想到CNN竟然能够学习到这一些的东西。确实是感叹一下了，敬佩！同时呢，我也是对这个可视化过程是非常的感兴趣的了，因为他就是会一下子让整个的内容高大上很多的了，以后确实是需要尽量做出从抽线到直观的图片了。这样子就是可以有很好的说服力了。</font>
...
VGGNet的卷积层有一个特点：特征图的空间分辨率单调递减，特征图的通道数单调递增，使得输入图像在维度上流畅地转换到分类向量。用通俗的语言讲，就是特征图尺寸单调递减，特征图数量单调递增。从上面的模型图上来看，立体方块的宽和高逐渐减小，但是厚度逐渐增加。<font color=green> （图18.7和Chapter 17中的这个顺序有点不一样，前面两个数是代表高度和宽度（我也不清楚哪个是高度，哪个是宽度），第三个数是channel数） </font>
...
3. 2个3x3卷积堆叠等于1个5x5卷积，3个3x3堆叠等于1个7x7卷积，感受野大小不变，而采用更多层、更小的卷积核可以引入更多非线性（更多的隐藏层，从而带来更多非线性函数），提高决策函数判决力，并且带来更少参数。<font color=red> 为什么2个3x3卷积堆叠等于1个5x5卷积？</font>
4. 每个VGG网络都有3个FC层，5个池化层，1个softmax层。
5. 在FC层中间采用dropout层，防止过拟合。

<font color=green> 前面三个小点的分析，解释还是非常的棒的，让我知道了一些chapter 17不知道的内容。</font>

虽然 VGGNet 减少了卷积层参数，但实际上其参数空间比 AlexNet 大，其中绝大多数的参数都是来自于第一个全连接层，耗费更多计算资源。在随后的 NIN 中发现将这些全连接层替换为全局平均池化，对于性能影响不大，同时显著降低了参数数量。

采用 Pre-trained 方法训练的 VGG model（主要是 D 和 E），相对其他的方法参数空间很大，所以训练一个 VGG 模型通常要花费更长的时间，所幸有公开的 Pre-trained model 让我们很方便的使用。<font color=red> ToDo: 很好奇，怎么样子使用这个pre-tranined model？这个应该有类似的教程的。</font>
...
图18-9的结构就是Inception，结构里的卷积stride都是1，另外为了保持特征响应图大小一致，都用了零填充。最后每个卷积层后面都立刻接了个ReLU层。在输出前有个叫concatenate的层，直译的意思是“并置”，即把4组不同类型但大小相同的特征响应图一张张并排叠起来，形成新的特征响应图。Inception结构里主要做了两件事：1. 通过3×3的池化、以及1×1、3×3和5×5这三种不同尺度的卷积核，一共4种方式对输入的特征响应图做了特征提取。2. 为了降低计算量。同时让信息通过更少的连接传递以达到更加稀疏的特性<font color=green>（这里的稀疏的概念，应该是类似于L1池化的概念了。）</font>，采用1×1卷积核来实现降维。
...
<font color=red> ToDo: 看前面三段的文字描述，多少还是有点吃力的，关于1x1的卷积在多通道的情况下可以降维（其实就是降低计算量），这个多少是可以看明白的。但是对于这个为什么用1x1的卷积核，不用3x3的卷积核，这的解释就是有一些吃力了。我觉得就是对于这个卷积层的参数维度的计算把握，还是不太好。

---->  

比如说上面的256×256×3×3后面跟的是卷积，然后作者也是说明了，大概是589000次乘积累加运算。举一个简单的例子，如果是1x3x3的数据，然后使用的是1x1x2x2的卷积核，乘积累加的次数是(4+3)x4=28；如果使用3x3x2x2=36，其实这两个数值也就是类似的。话句话说，如果是3x3x2x2的卷积，大概是需要执行28次乘积累加的了。

另外，上面AlexNet中只是提到了核(kernal)的概念，没有提到过滤器(filter)的概念，其实是可以这么理解的。如Section 17.1的图17-16所示，有一个3x32x32的输入数据，使用2x3x5x5的卷积核，得到的结果其实就是2x28x28的这么一个数据结构，其实在这么一个例子中，我只是需要说出卷积核的filter的维度数（即第一维度的具体数据），因为input data的第一维度(i.e. 3)是和卷积核的第二个维度(即每一个filter中kernal的数据,i.e. 3)是一致。我的理解，既然是在知道具体输入数据的维度的时候，我们就是可以忽略这个每一个filter中的kernal的描述了。
</font>

GoogLeNet网络结构中有3个LOSS单元，这样的网络设计是为了帮助网络的收敛。在中间层加入辅助计算的LOSS单元，目的是计算损失时让低层的特征也有很好的区分能力，从而让网络更好地被训练。在论文中，这两个辅助LOSS单元的计算被乘以0.3，然后和最后的LOSS相加作为最终的损失函数来训练网络。<font color=green> 多个LOSS单元他是这么操作的 </font>
...
<font color=green> 这个subsection给出了1x1的卷积核可以在多通道的输入数据中，降低计算复杂度的分析。</font>
...
图18-11 残差结构示意图 <font color=green>（图18-11中的F(x)其实就是上面文字部分的H(X) - X的概念了。） </font>

图18-12展示了两种形态的残差模块，左图是常规残差模块，有两个3×3卷积核组成，但是随着网络进一步加深，这种残差结构在实践中并不是十分有效。针对这问题，右图的“瓶颈残差模块”（bottleneck residual block）可以有更好的效果，它依次由1×1、3×3、1×1这三个卷积层堆积而成，这里的1×1的卷积能够起降维或升维的作用，从而令3×3的卷积可以在相对较低维度的输入上进行，以达到提高计算效率的目的。<font color=blue>（其实，这里也是提到了，就是这个1x1卷积核的作用，就是为了提高计算的效率的，这个也正是前面GoogLeNet中一种强调1x1卷积核的作用） </font>
...
DenseNet$^{[9]}$ 是一种具有密集连接的卷积神经网络。在该网络中，任何两层之间都有直接的连接，也就是说，网络每一层的输入都是前面所有层输出的并集，而该层所学习的特征图也会被直接传给其后面所有层作为输入<font color=green>（比如，根据图18.13中红色线，就是可以理解这个并集的含义了）</font>。下图是 DenseNet 的一个dense block示意图，一个block里面的结构如下，与ResNet中的BottleNeck基本一致：BN-ReLU-Conv(1×1)-BN-ReLU-Conv(3×3) ，而一个DenseNet则由多个这种block组成。每个DenseBlock的之间层称为transition layers，由BN−>Conv(1×1)−>averagePooling(2×2)组成。<font color=red>（todo: 不太理解最后一句话的意思呦。比如block的含义）</font>
...
何恺明先生在提出ResNet时做出了这样的假设：若某一较深的网络多出另一较浅网络的若干层有能力学习到恒等映射，那么这一较深网络训练得到的模型性能一定不会弱于该浅层网络。通俗的说就是如果对某一网络中增添一些可以学到恒等映射的层组成新的网路，那么最差的结果也是新网络中的这些层在训练后成为恒等映射而不会影响原网络的性能。同样DenseNet在提出时也做过假设：与其多次学习冗余的特征，特征复用是一种更好的特征提取方式。<font color=green> (非常的有意思呀，同样的假设，就是在ResNet & DenseNet都是提出来了，就是有类似于这样的理论证明吗？还是只是有一些的实验的结果呢？)</font>

<font color=red> ToDo: 这是第二次见到dense layer了，他和平常的有layer（比如全连接层，卷积层）有什么区别的呢？---> 直观的理解
Keras里的dense layer，就是全连结(fully connected)层。
Keras里的dropout layper，就是随机删去了一些连接，也就相当于是sparse layer。
</font>

<font color=green> 最后的两个网路，ResNet&DenseNet都“复用”这个概念的了。参见18.11-18.13中的曲线就是可以的，那个就是一个复用的概念。</font>
