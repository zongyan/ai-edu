...
<font color=blue> 从这个伪代码来看，早停法的本质，就是连续patience次“本次的loss大于上一次的loss”；当次数超过设定的数值的时候，就停止训练了。 </font>

#### 要注意的问题

<font color=green> 这里就是给出了这个门限值patience的选择指导了，一般最小是5，但是不超过30的了。可以在10，20，30进行选择。 </font>
...
<font color=red> ToDo: 得把这个training trace的代码看一下的了。 </font>
...
早停法并不会提高准确率，而只是在最高的准确率上停止训练（前提是知道后面的训练会造成过拟合），从上图可以看到，最高的准确率是99.07%，达到了我们的目的。<font color=blue> 这里就是很关键，早停法是不会提高准确率的，最多就是找到现在parameters情况下的最高准确率，然后就是在此刻停止继续训练了。 </font>
...
<font color=green> 也就是说，刚开始的早停（或者是本section上面提到的早停），都是基于这个validation set上面做的。但是呢，就是需要再次训练的时候，或者是下面的继续训练的时候，此时就是需要把validation set和testing set混合在一起的了。</font>

由于第一次早停是通过验证集计算loss值来实现的，所以这次不再分训练集和验证集，记住了早停时的迭代次数，可以重新初始化权重矩阵参数，使用所有数据再次训练，然后到达第一次的$i_{best}$时停止。

但是由于样本多了，更新批次也会变多，所以可以比较两种策略：<font color=red> ToDo: 但是我觉得这两个本质上是一回事情呀（只要不修改每一个epoch中batch的size了）。</font>
...
<font color=red> ToDo: 这个“再次训练” & “继续训练”的区别在哪里呢？都是训练，还是说这个hyperparameter修改了？还是怎么说？----> 不管是再次训练，还是继续训练，都是基于现在的参数，两个方法的相似之后，都是把evaluation data set和training data set混合在一起作为一个新的训练集。不同点在于，对于“再次训练”来说，然后训练到之前的$i_{best}$时停止。对于“继续训练”来说，还是使用evaluation data set进行监视loss function，看看能不能够获得更低的数值了。</font>
...
1. 本示例代码中给出的`patience`值为8，即有连续8次的loss值上升，就停止训练。试着修改这个值从5到20，看看对训练结果有什么影响。<font color=green> 感觉真的就是如作者所示，就是需要开始在patience=5的时候进行一点一点的慢慢实验，因为当数值等于11的时候，就是出现了过拟合的迹象了。</font>
