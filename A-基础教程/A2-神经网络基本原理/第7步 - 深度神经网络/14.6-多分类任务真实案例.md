...
<font color=green> 代码里面就是提供读取数据的方式，一种是按照image的方式读取，另外一种方式，就是按照每一个image由28x28的vector组成的形式。</font>
...
<font color=green> 从这里就是可以看出，输入总共就是有28x28=784个features了，在这一种情况下，并不是说第一层hidden layer的数量就是大于input features的数量的了。</font>

<font color=green> 参见Section 14.2，之前总结的hidden layer的神经元个数的内容，不全对。</font>
...
<font color=red> ToDo: 因为数据量太大的问题，所以这个MNIST的代码始终没有跑起来的了。</font>
