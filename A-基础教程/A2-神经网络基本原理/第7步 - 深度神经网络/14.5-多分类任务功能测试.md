...
用两层网络也可以实现，但是使用ReLE函数时，训练效果不是很稳定，用三层比较保险。<font color=red> 为什么这里使用了ReLU之后，两层的ReLU训练结果就是不太稳定呢？</font>
...
<font color=red> 这里给出了一些ReLU和Sigmoid的区别了。我就是也看了一下之前的一些情况，就是发现，在功能测试里面，都是使用的是Sigmoid激活函数，但是在实际案例中，都是使用的是ReLU激活函数的了，我不太清楚这个是为什么。</font>
<font color=green> 不过这里给出了ReLU和Sigmoid的区别，这个倒是一件非常好的事情。</font>
