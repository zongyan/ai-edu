...
<font color = green> 这里便是给出了这个L1范数为什么代替L1范数了，同时这个L1范数广泛应用的原因了。</font><font color = red> ToDo: 但是对这个凸优化的，解释不是特别的清楚，还是需要把这个部分的内容看一下，稍微了解一下的。</font>

### 16.3.3 L1正则化

<font color = red> ToDo: 但是有一个疑惑，为什么这个L2正则化里面，就是需要假定weights符合高斯分布，然后这里就是需要符合这个拉普拉斯分布（即双指数分布）？</font>
...
所以上述4个公式（4条直线）会组成一个二维平面上的一个菱形。<font color = green> 其实这里就是可以看出，为什么这个L1范数在二维平面是一个棱形，但是L2是一个圆形的原因的了。</font>
...
以最大的那个菱形区域为例，它与损失函数等高线有多个交点，都可以作为此问题的解，但是其中红色顶点是损失函数值最小的，因此它是最优解。<font color=green> 从这句话的了解，不管是L1正则，还是L2正则，选择的点都是需要距离椭圆心最近，意思就是说就是需要选择具体椭圆心最近的等高线上的点，等高线上的loss function的数值都是一样的。</font>
...
<font color=red> ToDo: 我现在就是产生了一个疑惑，就是到底是什么时候使用这个L1正则，什么时候使用L2正则。</font>
...
```
`train_y.shape[0]`就是样本数量。<font color=green> 这里就是有一个小的知识点，就是这个list是没有shape的，但是这个array是有这个shape的了，另外呢，因为python的index是从0开始的，所以就是只是显示第一个element。</font>
...
>>> array([ 1, -1,  1,  0])
```
当w为正数时，符号为正，值为1，相当于直接乘以w的值；当w为负数时，符号为负，值为-1，相当于乘以(-w)的值。最后的效果就是乘以w的绝对值。<font color=green> 这里就是解释了这个numpy.sign的用法了。numpy.sign returns an element-wise indication of the sign of a number. </font>

<font color=red> ToDo: 就是需要把反向传播的解释，以及代码都是过一下，现在突然就是有点模糊了。--> 参考Section 9.4</font>
...
从图16-18上看，无论是损失函数值还是准确率，在训练集上都没有表现得那么夸张了，不会极高（到100%）或者极低（到0.001）。这说明过拟合的情况得到了抑制，而且准确率提高到了99.18%。还可以画出拟合后的曲线与训练数据的分布做对比，如图16-19所示。<font color=red> ToDo: 就是很好奇，这个拟合的曲线，是怎么画出来的。--> 已经在Section 16.0的相应的commit中做出了注释</font>
...
<font color=blue> 这里给出了权重值较小，和趋近于0的一个清楚的解释了，较小的意思就是小于0.01的权重值，小于0.0001，就是意味着权重值接近于0。</font>

<font color=green> 这里就是给出了一个稀疏性的一个直观的表现，就是weights趋于0的个数了。</font>
...
<font color=green> 从这里我就是知道了两个关键性的信息，一个是这个`1e-4`就是一个非常小的数了，另外一个，`1e-2`是相对小很多的数了。借用作者的话来说，这个L1正则属于一枝独秀的结果，但是L2正则就是属于百花齐放的结果。但是我还是对什么情况下选择哪一个，没有一个明确的概念。不过稍微简单的看了一下第二个参考文之后，就是发现了，感觉L1正则在解释性方面就是更有优势，因为稀疏性的原因嘛，大部分的weights都是0了。</font>
...
1. 观察代码的打印输出的最后一部分，关于Norm1和Norm2的结果，仔细体会L1的作用<font color=green> 随着这个数值的$\lambda$从0.0001 -> 0.005 -> 0.01的变化，即使可以看到norm1和norm2都是变得越来越小的了。不管是L2正则，还是L1正则，就是norm1的变化最明显，但是nomr2的变化就是没有那么明显，到那时也是没有变法，norm1在over fitting情况下的数值就是很大，参见table16-4就是可以知道的了。</font>
2. 尝试改变代码中$\lambda$的数值，看看最后的拟合结果及准确率有何变化。<font color=green> 我在把$\lambda$调整到0.0001的时候，就是出现了过饱和的现象（就是和没有加L1正则项类似），然后调整到0.005的时候，基本上就是很好了，然后再调整到0.01的时候，就是这个L1正则项的效果有点过于强烈了。但是，总体的给我感觉，就是这个L1正则对$\lambda$的敏感度，没有L2高，这个是一件非常好的事情，在L2里面就是需要仔细的调整或者$\lambda$的参数，但是在$\lambda$里面就是会好很多，就是没有必要那么精心的调整了。</font>

<font color=red>但是，也是带来了一个问题，也就是需要凑这个$\lambda$的数值了？（不过，感觉从loss function@training data & evaluation data上来看的话，两个数值都是需要很小，丹斯对于evaluation data来说，又不能够出现这个U型曲线了。）</font>
