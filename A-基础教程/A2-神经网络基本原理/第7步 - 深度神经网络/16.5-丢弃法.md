...
<font color=red> ToDo: 这个高层的神经元，低层神经元，到底是一个什么意思呢？ --> 我的理解，高层的意思，就是越靠近这个输出层的隐藏层</font>
...
其中有叉子的神经元在本次迭代训练中被暂时的封闭了，在下一次迭代训练中，再随机地封闭一些神经元，同一个神经元也许被连续封闭两次，也许一次都没有被封闭，完全随机。封闭多少个神经元是由一个超参来控制的，叫做丢弃率。<font color=green> 也就是说，每一次迭代训练的过程中，哪一个神经元被封闭都是随机的。</font>
...
从公式3，Y将会是X经过r的mask的结果，1的位置保留原x值，0的位置相乘后为0。<font color=green> 这里就是给这个伯努利分布做了一个简单的介绍了，从中就是可以明白，主要就是通过这个变量$p$，比如是0.5，10个数中就是会随机出现5个1，然后5个0的。</font>
...
图16-26的右侧部分为测试/推理时，输入信号总会存在，但是在每个输出上，都应该用原始的权重值，乘以概率p。比如`input=1`，权重值`w=0.12`，`p=0.4`，则output$=1 \times 0.4 \times 0.12=0.048$。<font color=green> 这里非常的棒，就是给出了，在训练阶段，是可以使用这个dropout的，但是在测试或者推理阶段，就是不允许使用这个dropout的了，而是把信号乘以相应的概率，这个倒是非常有意思的。</font>
...
class DropoutLayer(CLayer):
    ...
    def forward(self, input, train=True):
        assert(input.ndim == 2)
        if train:
            self.mask = np.random.rand(*input.shape) > self.dropout_ratio #参见下面的解释
...
上面的代码中，`ratio`是丢弃率，如果`ratio=0.4`，则前面的原理解释中的`p=0.6`。<font color=green> 上面train中的代码是没有看明白，感觉是一个initialization的过程了。-----> 后来我就是通过debug明白了这行代码的作用了，他就是用来生成一个公式(2)的内容了，如果生成的随机数大于self.dropout_ratio，那么就是true，以及就是1，如果小于self.dropout_ratio。那么就是false，就是0。</font>
...
<font color=blue> 从图16-27 & 下面的代码示例来看，这个Drouput层是放在激活层和全连接层之间的。</font>
...
<font color=red> ToDo: 很奇怪，不管是从源代码部分，还是文本部分，都是把第二个隐藏层的dropout给取消了，我也不清楚他为什么会这么做的了。</font>
...
1. 由于每次用输入网络的样本进行权值更新时，隐含节点都是以一定概率随机出现，因此不能保证每2个隐含节点每次都同时出现，这样权值的更新不再依赖于有固定关系隐含节点的共同作用，阻止了某些特征仅仅在其它特定特征下才有效果的情况。<font color=green> 我倒是觉得这个解释是可以接受的。</font>
...
1. 分别调整3个dropout层的`drop_ratio`参数，观察训练结果的变化。<font color=green> 从调试的过程中，就是明白了，每一层的dropout rate都是可以不一样的；然后也并不是每一层都是需要有dropout layer的；想调整出一个比较好的dropout layer的配置，真的是需要花一些的时间的。如果就是仅仅是有一层隐藏层的话，就是在高层会好一些的了（同样的p的情况下），这个是我从实验中得到的一个现象了。</font>
2. 尝试改变`NeuralNet41.py`中的`inference`函数中的`output = self.__forward(X, train=False)`，把`train`设置成`True`，看看会得到什么结果。<font color=green> 就是会看到最后训练得到模型的结果是锯齿状的（红色线），这个主要就是因为把inference中的这个dropout功能打开了，才是会出现这个现象的；如果关闭，就是不会出现这个现象。或者换句话说，就是在testing or inference中，是不能够使用dropout功能的。</font>
