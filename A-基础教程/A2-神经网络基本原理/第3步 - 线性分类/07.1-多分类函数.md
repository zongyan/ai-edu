...
有的读者可能会有疑问：我们不能训练神经网络让它的z值直接变成概率形式吗？答案是否定的，因为z值是经过线性计算得到的，线性计算能力有限，无法有效地直接变成概率值。<font color="red"> ToDo: 为什么会想出这个问题，我就是不会想出来的呢？-->比如在二分类中，使用logistics function训练出来的数值是在$[0, 1]$之间，我们就是可以把这个数值理解成为一个概率的数值，其实本质上，他不是一个概率的含义。--> 是我自己理解错了，如Section 6.2中所示，logistics function本身就是一个概率函数，所以经过logistics function之后，比如数值越是接近1，说明正例的可能性越大。-->但是，我是认为，对于Tanh function来说，就是不能够用概率这个解释的了，因为Tanh得到的数字是$[-1, 1]$之间的了，解决方案是类似于Logistics function，但是就不能够用概率问题来进行描述的了。</font>

#### 取max值

z值是 $[3,1,-3]$，如果取max操作会变成 $[1,0,0]$，这符合我们的分类需要，即三者相加为1，并且认为该样本属于第一类。但是有两个不足：<font color="red"> ToDo: 这个是怎么得到$[1,0,0]$，我暂时没有想明白。---> numpy中是存在这么一个函数argmax的，他是可以找出最大的值，以及其对应的index的。</font>
...
也就是说，在（至少）有三个类别时，通过使用Softmax公式计算它们的输出，比较相对大小后，得出该样本属于第一类，因为第一类的值为0.879，在三者中最大。注意这是对一个样本的计算得出的数值，而不是三个样本，亦即Softmax给出了某个样本分别属于三个类别的概率。<font color="green"> 这一段话的描述是非常的棒的了，也是适用于二分类问题的了。 </font>
...
是不是和Logistic函数形式非常像？其实Logistic函数也是给出了当前样本的一个概率值，只不过是依靠偏近0或偏近1来判断属于正类还是负类。<font color="green"> 之前讲Logistic函数是概率函数的时候，我没有太留意，但是这里就是着重点到了这个问题，然后我也是意识到了。但是，这个概率的描述方式，对于Tanh函数是不适用的。</font> <font color="red">ToDo: 这里一直是和max进行对比，我还是不明白这个max的具体function是一个什么，但是我是理解了softmax的了。 </font>
...
#### 损失函数计算 （<font color="green"> 这个和交叉熵损失函数还是有一点区别 </font>）
...
- 求输出项 $a_2$ 或 $a_3$ 对输入项 $z_1$ 的导数，此时：$j$ 为 $2$ 或 $3$, $i=1,i \neq j$，可以扩展到 $i,j$ 为任意不等值 <font color="green"> 求 $a_2$ 或 $a_3$ 对 $z_1$ 的导数， 因为$z_1$作为输入，也是把数值传递到了 $a_2$ 和 $a_3$ 里面的了，所以也就是需要求导的了。</font>
...
看上图，我们要求Loss值对Z1的偏导数。和以前的Logistic函数不同，那个函数是一个z对应一个a，所以反向关系也是一对一。而在这里，a1的计算是有z1,z2,z3参与的，a2的计算也是有z1,z2,z3参与的，即所有a的计算都与前一层的z有关，所以考虑反向时也会比较复杂。<font color="green"> 作者这里就是提到了，对 $z_1$ 进行求导的时候，为什么就是需要 $a_2$ 或 $a_3$ 对 $z_1$ 求导。</font>
...
其含义是，样本预测第一类，但实际是第二类，所以给第一类0.879的惩罚值，给第二类0.881的奖励，给第三类0.002的惩罚，并反向传播给神经网络。<font color="green"> 对于loss的error计算，是使用prediction - label这个公式，这个得到的数值，就是会被用到计算weights和biases上面的了，就是weights和biases就是会减去上面这个数值的了。 如果prediction - label是正数的话，就是减；如果是负数的话，就是会加，所以，正数就是惩罚；负数就是奖励。</font>

<font color="green"> 从公式（20）到这里都是一些分析解释的了（比如解释为什么就是需要 $a_2$ 或 $a_3$ 对 $z_1$ 求导，以及一些基本的求导知识），如果公式（20）就是能够看明白的话，这个部分的内容就是可以扫过去就是可以的了。</font>
...
<font color="green"> 这里公式的推导，是对上面Softmax2的定义方式的证明。</font>
...
`axis=1`这个参数非常重要，因为如果输入Z是单样本的预测值话，如果是分三类，则应该是个 $3\times 1$ 的数组，如果：<font color="green"> 这个的作用，就是每一行的数据相加。</font>
...
```
A虽然仍然包含两个样本，但是变成了两个样本所有的6个元素相加为1，这不是softmax的本意，softmax只计算一个样本（一行）中的数据。如果：

<font color="green"> 这句话非常的好，就是给出了softmax处理数据的特点。</font>

<font color="green"> 对于加入了`axis=1`，得到的`np.sum(exp_z, axis=1, keepdims=True)`的结果就是一个$2 \times 1$的变量了。但是呢，对于`np.sum(exp_z,keepdims=True)`这个运算，得到的结果就是仅仅是一个标量。然后`a = exp_z / np.sum(exp_z, axis=1, keepdims=True)`是一个类似于element-wise的运算（具体点说，`exp_z`的第一行数据就是和`np.sum(exp_z, axis=1, keepdims=True)`的第一行数据进行相除的，第二行也是类似的）。这样子得到的结果。</font>

<font color="green"> 这个debug功能用好，真的是一件事倍功半的效果了，参见 ch07, Level0 （这个代码是我把这一节最后的内容拷贝进去研究的，要不然就是不容易看明白）。</font>
...
<font color="red"> ToDo: 真的有一种可能性的存在，对于这一种，倒是好办，因为我本来就是需要概率最大的那一个数值，但是$[0.4,0.4,0.2]$这一种，我就真不知道应该是怎么解决了（因为比如Section 7.0中，这一种情况下，就是不太好确定是哪一个类别）。

但是，如果把这个问题是回到Section 7.0的情况了，我暂时还是不清楚怎么用这里的方法解决的了（就算是最普通的一种情况了）。

--> 看了Section 7.1刚开始的一段话，我就是明白了，就是会输入横轴&纵轴的两个特征值信息，然后就是可以得到包含三个元素的矩阵，这个矩阵中的信息，就是代表了属于三种不同阵营的概率。所以就是可以根据这三个元素，来进行判断了。同时呢，如果对于第一种情况$[0.3,0.3,0.4]$，我的理解是非常不需要任何的困难的了，他就是属于第三类；但是对于这个$[0.4,0.4,0.2]$，我也不清楚应该怎么处理的了。</font>


<font color=green> softmax可以认为是输出层的“激活函数”（这个词有点不准确，但是可以帮助理解）。所以输出层的神经元数值z经过softmax层之后，就是会得到a，这个数值是可以用来判断是属于哪一个分类的（具体参见公式1以上的文字解释）。然后这个数值，经过loss function之后，也是可以反向传播给神经元的（参见公式5-20的推导过程）。我同时也是明白了，为什么在分类问题里面，就是不需要除以这个样本数$m$的了。同时公式2的运算就是可以知道，其实对于每一个分类，其数值已经组了归一化的处理的了，另外，作者自己也是明确了的表明了，使用了softmax之后，所有分类的总和等于1。这个和均方差损失函数是完全不一样的，均方差损失函数里面，本身就是有一个平方，所以就是需要除以样本数$m$的了，避免梯度爆炸。</font>
