...
<font color='green'>这里就是使用class枚举的示例还是需要学习一下的，这个之前在C++经常性的出现，因为Python也是面向对象的语言，也是会有类似的操作的了。</font>
...
分类的方式是，可以指定当 $A>0.5$ 时是正例，$A\leq 0.5$ 时就是反例。有时候正例反例的比例不一样或者有特殊要求时，也可以用不是 $0.5$ 的数来当阈值。<font color='red'> ToDo:这个$0.5$的分类阈值，在代码里面，是怎么实现的呢？ --> NN返回的数值是一个A，这个阈值，不是代码加进去的，而是后天代码加进去的。</font>
