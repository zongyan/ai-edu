...
- 从视觉上判断是线性可分的，所以我们使用单层神经网络即可<font color="green"> 基本上就是可以认为，线性问题单层网络就是可以的了，但是对于多层网络，就是可以解决非线性的问题</font>
- 输入特征有两个，分别为经度、纬度
- 最后输出的是三个分类，分别是魏蜀吴，所以输出层有三个神经元

<font color="green"> 参见上一节最后的一个小问题的回应，就是大概可以明白上一节的内容，应该是怎么应用到三分类（甚至于多分类）的问题了。</font>
...
<font color="green"> 一个分类，是对应一个神经元的。</font>

<font color="red"> ToDo: 其实这一段话就是已经解释的很清楚了，但是就我自己而言，还是感觉是有一些的模糊的，希望看完这一整个section，就是可以解决我的模糊。</font>
...
与前面的单层网络不同的是，图7-7最右侧的输出层还多出来一个Softmax分类函数，这是多分类任务中的标准配置，可以看作是输出层的激活函数，并不单独成为一层，与二分类中的Logistic函数一样。<font color="green"> 原来这个就是常识性的问题，即softmax就是用在多分类任务中，而logistics function则是使用在二分类任务中。另外，之类的soft max & logistics functions不是单独作为一层使用的。</font>
...
$W$权重矩阵的尺寸，可以从前往后看，比如：输入层是2个特征，输出层是3个神经元，则$W$的尺寸就是 $2\times 3$。<font color="green"> 这里的 $W$ 矩阵的设计，线性回归中 $W$ 的设计是一样的，关于线性回归中的各种矩阵的设计，请参见Section 4.5。</font>
...
<font color="red"> ToDo: 按照这里的理解，这个softmax函数，是放在输出层的。</font>
...
在有Softmax的多分类计算时，我们用下面这种等价的方式，俗称OneHot。<font color="green"> 这个就是需要注意一下。</font>
$$
Y = 
\begin{pmatrix}
y_1 \\\\ y_2 \\\\ \vdots \\\\ y_{140}
...
函数`np.argmax`的作用是比较`output`里面的几个数据的值，返回最大的那个数据的行数或者列数，0-based。比如output=(1.02,-3,2.2)时，会返回2，因为2.2最大，所以我们再加1，把返回值变成1,2,3的其中一个。<font color="green"> 这个函数好，就是避免了以后我自己还是需要寻找的了。</font>

`np.argmax`函数的参数`axis=1`，是因为有4个样本参与预测，所以需要在第二维上区分开来，分别计算每个样本的argmax值。<font color="red"> ToDo: 需要在debug的时候，搞清楚这个问题的了。 --> 参见源代码的解释。</font>
...
<font color="red"> ToDo: 这个 $r$ 的数值是怎么得到的，需要在代码里面搞清楚的了。--> $r$ 的数值就是其实就就是matrix位置的含义了，然后是用`np.argmax`的方式得到的。</font>
...
1. 从4个样本的推理结果来看，分类都是正确的，但是只有第一个样本的结果的0.734的概率值处于绝对领先位置，其它几个分类的概率值优势并不明显，这是为什么？如何让其正确分类的概率值的差距更大？ <font color="green"> 我看了一下源代码，觉得是因为max_epoch的数值太小了(100)，我换成10000之后，就是解决了这个问题了。Note，其实这个训练的精度设置的还是可以的（1e-3），所以就是不需要调整。</font>
