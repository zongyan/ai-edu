...
2. GRU不保留单元状态 $c_t$，只保留隐藏状态 $h_t$作为单元输出，这样就和传统RNN的结构保持一致。<font color=green>这个真的是一件非常好的事情，感觉LSTM实在是太复杂了。</font>
...
然后，通过更新门实现遗忘和记忆两个部分。从隐藏状态的公式可以看出，通过$z_t$进行选择性的遗忘和记忆。$(1-z_t)$和$z_t$有联动关系，上一时刻信息遗忘的越多，当前信息记住的就越多，实现了LSTM中$f_t$和$i_t$的功能。<font color=green>这段文字，算是把上面的四个公式都是解释的非常的清楚的了。</font>

### 20.3.3 GRU的反向传播

学习了LSTM的反向传播的推导，GRU的推导就相对简单了。我们仍然以$l$层$t$时刻的GRU单元为例，推导反向传播过程。

同LSTM， 令：$l$层$t$时刻传入误差为$\delta_{t}^l$，为下一时刻传入误差$\delta_{h_t}^l$和上一层传入误差$\delta_{x_t}^{l+1}$之和，简写为$\delta_{t}$。<font color=green>参见Section 20.2</font>

令：<font color=green>因为公式1-4是这个加入activation function之后的表达式，为了方便起见，这里就是重新写了下面三个公式</font>
...
则：<font color=green>就算是图20-7没有给出具体的各个变量的关系，但是根据公式1-4的表达式，还是可以很容易的推导出相应的误差矩阵的。</font>
...
当前GRU cell分别向前一时刻（$t-1$）和下一层（$l-1$）传递误差，公式如下：<font color=green>根据Section 20.1的公式37, 38，以及图20-4就是可以明白，在计算“沿时间向前传递”和“沿层次向下传递”的时候，其实都是需要借助于公式5-7的三个变量。或者借助于公式1-4变量直接的关系，一下子也是可以理解的了。</font>
...
<font color=red> 这个部分的代码暂时就是没有修改，实现的了。如果有必要，以后就是实现一下的了。不过实现之前，估计先是需要把Section 20.1-20.2的内容回顾一下，这样子就是方便上手的了。</font>