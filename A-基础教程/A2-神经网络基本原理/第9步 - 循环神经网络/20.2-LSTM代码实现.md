...
上一节，我们学习了LSTM的基本原理，本小节我们用代码实现LSTM网络，并用含有4个时序的LSTM进行二进制减法的训练和测试。<font color=red> 根据Section 20.0的内容，所以本小节的代码例子是输入和输出序列等长？即输入是4，输出也是4 </font>
...
初始化时需要告知LSTM Cell 输入向量的维度和隐层状态向量的维度，分别为$input\_size$和$hidden\_size$。<font color=red> 看一下代码中，$input\_size$和$hidden\_size$的具体数值，然后心里大概就是有一个数了。</font>
...
其中，$get\_params$将传入参数拆分，每个门使用一个独立参数。$get\_gate$实现每个门的前向计算公式。<font color=red> 我不太明白这个$get\_gate$的作用 --> 参见上面这一段代码，就是可以明白这个$get\_gate$的作用了。</font>
...
1. 本例中都使用了bias参数，如果去掉bias（即bh，b皆为0）效果如何。<font color=green> accuracy上面看并没有特别大的区别，最后的精度也确实是1了。不过让我惊讶的是，去掉bias之后，这个训练的速度（或者说是iteration的数量）明显减少了很多。</font>
2. 用不同的方式初始化权重W，U，比较不同的结果。<font color=green> 通过代码，我额外添加了另外两种初始化方式，一个是Xavier，另外一个Xavier，比较代码中的四种初始化方式，都是能够训练成功的，唯一的不同仅仅是训练的速度。ToDo：需要查看其他的文献，看看其他的文献到底都是怎么说这件事情的。</font>
3. 使用不同batch_size，比较不同结果。<font color=green> 使用原始的初始化方式，保留bias。然后batch size从1增加到2，精度并没有什么太大的变化，唯一有区别的就是这个训练速度的了。另外，我增加到4的时候，就是没有办法训练成功的了，可能是和变量的维度有关系</font>

<font color=red> 对于第二个练习，就是需要查看以下资料的了，看看其他人是怎么说的，对于第三个问题，我并不认为有任何的代表性。仅仅是针对这个问题而言的</font>
