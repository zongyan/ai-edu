...
具体地描述此问题：当接收端接收到两个连续的值，如0.35、0.46时，系统响应为0.35；下一个时间点接收到了0.12，考虑到上一个时间点的0.46，则二者组合成0.46、0.12序列，此时系统响应为0.46；依此类推，即接收到第二个数值时，总要返回相邻的第一个数值。<font color=green> 我实在是没有看明白这段话的描述，我的理解，就是Y是X延迟了1个时间单位（即1秒）的数据点。----其实，之所以总是按照这个两个数的组合进行解释分析，还是因为在这个section里面，作者的假设就是两个时间步的RNN，所以，就是为了贴合标题，就是输入两个数据到RNN中的时候，就是会返回第一个数据的了。----注意一下，这里的主语是“接收端”</font>
...
举个例子来说，x[10, 5, 4] 表示第10个样本的第5个时间点的第4个特征值数据。<font color=red> 需要根据代码，理解数据的结构，以及怎么样子把数据送到rnn的。----->其实就是多了一维，之前的dnn中，都是二维的数据，第一维度是样本，第二维度是特征值；cnn中，可以理解成为某种程度上的二维，第一维度是样本，第二维度是特征值（只不过这个特征值是一个三维的数据，high x width x channel）。rnn中，区别就是在于存在另外一个维度，就是时间维度（上面的第二维度）。</font>
...
我们回忆一下，在验证万能近似定理时，我们学习了曲线拟合问题，即带有一个隐层和非线性激活函数的前馈神经网络，可以拟合任意曲线。但是在这个问题里，有几点不同：<font color=green> (这里是说明rnn和之前的cnn&dnn的不同之处) </font>

1. 不是连续值，而是时间序列的离散值 
2. 完全随机的离散值，而不是满足一定的规律
3. 测试数据不在样本序列里，完全独立 <font color=red> (这一点我不太明白) </font>
...
图19-8中含有两个简单的前馈神经网络，t1和t2，每个节点上都只有一个神经元，其中，各个节点的名称和含义如表19-3所示。<font color=green>图中的网络，其实是一个双层网路，一个隐藏层，一个输出层 </font>
...
请读者注意，在很多关于循环神经网络的文字资料中，通常把h和s合并在一起。在这里我们把它们分开画，便于后面的反向传播的推导和理解。<font color=blue> 这个点是非常的重要的，就是需要注意一下的了。</font>

还有一个问题是，为什么t1的后半部分是虚线的？因为在这个问题中，我们只对t2的输出感兴趣，检测t2的输出值z和y的差距是多少，而不关心t1的输出是什么，所以不必计算t1的z值和损失函数值，处于无监督状态。<font color=red> 那可以这么理解不？就是在RNN中，如果类似于本section的这一种情况，或者是多对一的情况，那么输出都是在高时间步对应的神经元上（比如本section中的t2）</font>
...
本来整体的损失函数值$loss$应该是两个时间步的损失函数值之和，但是第一个时间步没有输出，所以不需要计算损失函数值，因此$loss$就等于第二个时间步的损失函数值。<font color=red> todo: 所以就是在rnn中，都是应该是使用所有的时间步的loss的和来进行训练吗？---> 这个section里面的代码，因为只有一个输出，所以也就是使用的全部loss进行训练的。 </font>
...
<font color=blue> 这里确实是和FFNN, CNN的更新梯度的方式确实是不一样的，同时，也是给出了，因为是公用这个$U、V、W、bz、bh$，所以就是需要使用来自于t1 & t2的两个神经元的相关参数的了。另外，(9)-(13)，(15)-(19)是对应的梯度(矩阵)的计算，(7),(8)&(14)对应的误差(矩阵)的计算。</font>
...
在训练函数中，仍然采用DNN/CNN中学习过的双重循环的方法，外循环为epoch，内循环为iteration，每次只用一个样本做训练，分别取出它的时序1和时序2的样本值和标签值，先做前向计算，再做反向传播，然后更新参数。<font color=red> ToDo: 需要review一下FFNN & CNN的代码了，我对这个没有印象，比较尴尬。---->对的，确实是两个循环，外面一个循环就是epoch了，就是需要遍历max_epoch次的train data，内循环就是iteration（每一次train data set都是被分成max_iteration个batch(max_iteration=train_data_size/batch_size)），每一次的内循环，就是使用batch的数据进行训练的了。</font>
...
从图19-9的训练过程看，网络收敛情况比较理想。由于使用单样本训练，所以训练集的损失函数变化曲线和准确度变化曲线计算不准确，所以在图中没有画出，下同。<font color=red> 单样本训练？就是单个样本训练出来的结果，但是我对这句话还是不能够理解。---> 意思就是说，在训练的时候，batch_size 设定的是1，这样子每次训练，仅仅是使用一个样本，所以这种情况下，抖动就是比较的大的了。</font>
...
可以看到W的值比其他值大出一个数量级，对照图19-8理解，这就意味着在t2上的输出主要来自于t1的样本输入，这也符合我们的预期，即：接收到两个序列的数值时，返回第一个序列的数值。<font color=green> 关于这个w的解释&分析，还是非常的到位的。就是把本节提出的问题，以及就是这个训练得到的W结合在一起了。</font>

至此，我们解决了本章开始时提出的问题。注意，我们没有使用到循环神经网络的任何概念，而是完全通过以前学习到的前馈神经网络的概念来做正向和反向推导。但是通过t1、t2两个时序的衔接，我们已经可以体会到循环神经网络的妙处了，后面我们会用它来解决更复杂的问题。<font color=green> 这就是需要记住的是，本section暂时没有接触到循环的概念，或者到目前为止，理解的循环的概念，就是从t1的激活层向t2的隐藏层输出数据。</font>

<font color=red> 我暂时也是没有办法体会到这个所谓的rnn的妙处，我得先看一下这个数据是怎么输入到rnn中的，看看是不是有点感悟。--> 就算是我看懂了这个输入数据的维度等问题，但是我还是没有体会到这个rnn的妙处所在。</font>
...
<font color=green> 因为作者自己也是提到所有的变量都是标量，所以在这个section里面暂时是不会考虑矢量的问题了，所以也就是不会考虑转置等现象了。

在代码中，train data的维度是200x2x1，label data的维度是200x2；对于x输入来说，每次就是取一组数据（其维度是1x2x1），然后把第一个数据输入到t1的x，把第二个数据输入到t2的x。这样子就是可以进行前向计算。类似的，对于反向传播，就是把y的第一个数据输入到t1的反向传播函数（但是函数中并没有使用这个数），把y的第二个数据输入到t2的反向传播函数。

然后在实验中，对于train data来说，其维度是200x2x1，第一维度代表的是样本数，第二维度代表的是时间步，第三维度代表的是特征值数量，也就是说，在这个程序里面，总共有200个样本，每一个样本就是有两个时间步，然后每一个时间步仅仅是只有一个特征值的了。
</font>
