...
异或数据比较简单，只有4个记录，所以就hardcode在此，不用再建立数据集了。这也给读者一个机会了解如何从`DataReader`类派生出一个全新的子类`XOR_DataReader`。<font color=green> 这个派生的问题在前面我就是搞明白了，感觉我的学习效果还是很好的，就是主动学习（主动思考），不是跟着他一点一点的学（被动思考）。</font>
...
与第6章中的逻辑与门和或门一样，我们需要神经网络的运算结果达到一定的精度，也就是非常的接近0，1两端，而不是说勉强大于0.5就近似为1了，所以精度要求是误差绝对值小于`1e-2`。 <font color=green> 他这里就是非常的棒的，给出了这个`1e-2`的精度理由了。</font>
...
<font color=red> ToDo: 这个numpy.where代码不太清楚是一个什么意思，这个是第一次遇到。 --> 参见代码部分的comments，能够理解这个代码，但是暂时不太清楚可以用到那里。</font>
...
- `n_input = dataReader.num_feature`，值为2，而且必须为2，因为只有两个特征值<font color=green>（特征值，在ML里面，就是会用这个feature来进行表示。）</font>
...
可以看到二者的走势很理想。<font color=green>我觉得这里可以学习的一个点，便是在于这个准确率accuracy的计算，意思就是说，这里不仅仅是把loss的趋势给打印出来了，还是把这个准确率accuracy的数值给打印出来了。</font> <font color=red> ToDo: 同时，这里就是需要记住，在ML中（或者说在本书中），accuracy，precision的定义都是不一样的，不可以随便的乱用，详细的内容请参见section 10.0。</font>
