...
<font color=green> 
这个训练的过程好难呀，就是想找到一个合理的参数，我先是调整了这个hidden neurons的数量，随着这个数量的增加（数量=9，max_epoch=10000），确实是会好很多，但是太多的时候，就又是会变糟糕的了。

得到一个相对满意的hidden neurons的数量之后，我再基于这个9的数据，eta学习率试试看，在0.05的时候，感觉学习率还是可以的，然后再继续减小到0.01，就是性能是变差了。这里需要注意的是，如果调整了学习率，就是需要关注max_epoch的数值，关注loss函数的收敛情况，尽量就是让他是因为达到了eps精度再停止训练，而不是因为达到了这个max_epoch的最大值。

然后我接着就是继续把eps精度调整到0.05，这个训练之后的性能就是变得更加好的了。

也就是说，我训练时使用的hyper parameter如下：n_hidden = 9, eta, batch_size, max_epoch = 0.05, 10, 30000, eps = 0.05。（在epoch=23399停止训练的）

我觉得，就是在这个训练的hyperparameter上面就是缺少经验的问题有点暴露了，也就是实际工程经验不足吧。不太清楚最好先是调整那个hyper parameter，然后再调增哪一个的了。
</font>

<font color=green> 
For non-linear regression, one hidden layer and one output layer are used, there is an activation sigmoid function in the hidden layer.

For non-linear two classification, one hidden layer and one output layer are used, there is an activation sigmoid function in the hiddern layer, and a logistic function in the output layer.

For non-linear multiple classification, one hidden layer and one output layer are used, there is an activation sigmoid function in the hiddern layer, and a softmax function in the output layer.</font>

<font color=red> 
ToDo: 从代码的角度，对比一下这三种不同的非线性问题，在层数的定义，激活函数，以及函数的使用上的区别。--> 代码部分，就是class的forward函数里面确实是如此的。</font>
