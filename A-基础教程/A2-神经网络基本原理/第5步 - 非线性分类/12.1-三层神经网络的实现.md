...
<font color=red> ToDo: 这里就是一件非常有意思的事情，hidden layer 1使用的是sigmoid activation function，hidden layer 2使用的是tanh activation function, output layer使用的是Softmax function。这个输出层可以理解的，但是就是在隐藏层的时候，就是会使用两种不同的activation function的呢？ 到目前为止，我看到最多的，就是使用这个sigmoid function的了。就算是使用这个sigmoid function，也是在Z1（扩大尺寸之后），然后就是使用activation function，让多个种类群区分开来，如Section 10.6所示。</font>

### 12.1.3 反向传播

和以前的两层网络没有多大区别，只不过多了一层，而且用了tanh激活函数，目的是想把更多的梯度值回传，因为tanh函数比sigmoid函数稍微好一些，比如原点对称，零点梯度值大。

<font color=red> ToDo: 这里我不太明白，确实是零点梯度值大，但是为什么说更多的梯度值回传呢？--> 看Section 8.1的结果，我的感觉，就是这个Tanh的这个导数的数值范围是比这个Logistic大的，这个是不是就是更多梯度值回传的意思的呢？

另外，在Section 8.1中，也是提到了，这个tanh确实在很多的应用中比这个logistic好，这个就是需要再看看文献的了。</font>

#### 输出层

$$\frac{\partial loss}{\partial Z3} \rightarrow dZ3 
= A3-Y \tag{7}$$
$$\frac{\partial loss}{\partial W3} \rightarrow dW3 = A2^{\top} \cdot dZ3 \tag{8}$$
$$\frac{\partial loss}{\partial B3} \rightarrow dB3=dZ3 \tag{9}$$
...
<font color=green> 如果对上面这个部分的公式有任何的疑惑，请参见Section 7.1 and Section 11.1。</font>
...
<font color=red> ToDo: Section 9.4中，当时也是遇到了这个/m的问题，但是一直也是没有明白为什么不除，就是可以会造成梯度爆炸。就算是我现在返回去看这个问题，还是不明白为什么会有这个除法的。---> 后来就是在使用4个样本的例子，然后就是在Section 9.4中解决了这个疑惑的了。但是，也是因为多样本的原因，比如就是在回归问题中，就是出现了这个4x1的维度，就是说每一个样本就是对应一个收敛的数值，不太清楚这个是不是正确的。---> 但是想想，感觉这个思考是错误的，比如说在house price的这个问题里面，最后得到的结果就是仅仅是一个数值，看看后面能不能通过debug代码解决这个额问题的了。</font>
...
<font color=red> ToDo: 很遗憾，暂时没有找到这个的数据集，所以就是没有办法把代码跑起来的了。--> 后来我就是在网站上面找到了数据集，但是就是出现了报错的现象，就是因为数据集太大了，然后就是超过了array的最大值了，这个问题就是相对轻松一些的了，只要是减小文本阅读量就是可以的了。--> 虽然是因为数据集的原因，暂时是没有办法用debug的形式把代码过一遍的了，但是我自己还是手动的方式就是把代码过了一遍的了。</font>
...
<font color=green> 第一个问题很简单，因为是使用了三层的网络，所以就是没有必要说是第一层的hidden neurons的数量，是超过784个的了，对于第二个问题，我倒是很好奇，就是想着是通过修改代码实现的，但是无奈是没有数据集，这个倒是一个非常坑爹的问题了。</font>
