...
<font color="red"> ToDo: 这个点是我不太清楚的，为什么归一化特征值之后，得到的loss function， $W$， $B$ 都是应该在$[0,1]$呢，我还是没有想明白。</font>

大数量级的数据有另外一个问题，就是它的波动有可能很大。目前我们还没有使用激活函数，一旦网络复杂了，开始使用激活函数时，像486.166这种数据，一旦经过激活函数就会发生梯度饱和的现象，输出值总为1，这样对于后面的网络就没什么意义了，因为输入值都是1。<font color="green"> 这个倒是肯定的，就是因为loss function的数值一旦是放到了激活函数之后，就是会出现梯度饱和的现象。</font>
