...
<font color="green">对于利用链式法则求 $w$ 和 $b$ 梯度合理性的问题，就是在这个2.1已经解释了，也是给出了一个的案例的了。对于这个梯度的解释， 当 $w$ 变化一点点时， loss function 会产生 $w$ 的变化值（一个梯度数值）倍的变化 [之所以这里是loss function，因为在这一个小结里面，是 loss function 对 $w$ 的求导]</font>
...
<font color="red">ToDo: 我这里就是有一个疑惑，比如说，NN的训练中，就是不需要这个Hypothesis Function的了，直接是把data放进去训练就是可以的了；但是呢，到目前为止，就是为这个梯度下降来说，都是一直是需要这个Hypothesis Function的了，这个就是和我的认知有一定的区别的</font>