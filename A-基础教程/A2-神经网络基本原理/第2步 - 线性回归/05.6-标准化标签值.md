...
1. $X$ 必须标准化，否则无法训练；
2. $Y$ 值不在 $[0,1]$ 之间时，要做标准化，好处是迭代次数少 <font color="green"> （因为eta学习率可以很大，然后就是加快学习速度， 如果不归一化标签值，此时loss function的数值就是很大，学习率就是需要调得非常的小）。</font>；
...
<font color="red"> ToDo: 暂时先不继续往下看了，先是把代码过一遍，明白代码是怎么写的，再继续往下写，要不然就是怕消化不了。</font>

<font color="red"> ToDo: 不太清楚这个ch05, Level7的代码是干嘛的</font>