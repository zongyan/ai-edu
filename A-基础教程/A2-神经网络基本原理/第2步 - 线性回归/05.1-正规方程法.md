...
<font color="green"> 公式（9）和（10）求逆矩阵的这个思路在之前的ELM中确实是遇到过，我之前是不知道为什么总是会出现$X^{\top} X$这么一个term，现在终于是明白了，就是为了防止矩阵$X$不是方阵，所以就是通过上面的这一种方式变成方阵，然后接着就是可以求得这个方阵的了。</font>
...
把 $b$ 看作是一个恒等于 $1$ 的feature，并把 $Z=XW$ 计算公式带入，并变成矩阵形式：<font color="green"> $n$ 代表特征值的个数，$m$ 代表样本的个数。</font>

$$J(W) = \sum_{i=1}^m \left(\sum_{j=0}^nx_{ij}w_j -y_i\right)^2=(XW - Y)^{\top} \cdot (XW - Y) \tag{12}$$

<font color="green"> 如果想求如（12）中的每一个元素的平方和，转换成矩阵的形式，就是矩阵的逆 乘以 矩阵本身 即可。</font>
...
<font color="red"> ToDo: 关于公式（13）的矩阵求导，以及关于 逆矩阵$(X^{\top}X)^{-1}$ 是否存在的情况分析，我目前觉得就是没有必要深究了，只要抓住主线就是可以的了。如果真的是需要深究的话，就是需要看项目，或者说是文章的需要了。</font>
...
<font color="green"> 第一列的元素，请参见（5）就是可以的了。</font>
...
<font color="green"> 其实这个代码，可以作为以后开发ELM的一个很好的样板，因为我是觉得，虽然里面是很多的内容都是不一样的，但是这个基本的programming syntax for matrix是可以借鉴使用的。</font>
